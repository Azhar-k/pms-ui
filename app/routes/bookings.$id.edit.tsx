...
export default function EditBookingPage() {
  ...
  const [checkInDate, setCheckInDate] = useState<string>(
    reservation.checkInDate ? new Date(reservation.checkInDate).toISOString().split('T')[0] : ""
  );
  const [checkOutDate, setCheckOutDate] = useState<string>(
    reservation.checkOutDate ? new Date(reservation.checkOutDate).toISOString().split('T')[0] : ""
  );
  ...
  useEffect(() => {
    const fetchAvailableRooms = async () => {
      if (checkInDate && checkOutDate) {
        ...
        try {
          // Include the current room in available rooms for editing
          ...
          const currentRoomInList = availableRooms.some((r: any) => r.id === reservation.roomId);
          if (!currentRoomInList && reservation.room) {
            setRooms([reservation.room, ...availableRooms]);
          } else {
            setRooms(availableRooms);
          }
        } catch (error) {
          console.error("Error fetching available rooms:", error);
          // If error, at least show the current room
          if (reservation.room) {
            setRooms([reservation.room]);
          } else {
            setRooms([]);
          }
        } finally {
        ...
        // If dates are not selected, load current room
        if (reservation.room) {
          setRooms([reservation.room]);
        } else {
          try {
    ...
    fetchAvailableRooms();
  }, [checkInDate, checkOutDate, reservation.roomId, reservation.room]);
        ...
        <Form method="post" className="space-y-6">
          ...
          <div>
            ...
            <select
              id="roomId"
              name="roomId"
              defaultValue={reservation.roomId}
              required
          ...
          <div>
            ...
            <select
              id="rateTypeId"
              name="rateTypeId"
              defaultValue={reservation.rateTypeId}
              required
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-gray-900 bg-white"
            >
